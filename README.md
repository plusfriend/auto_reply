...
안녕하세요. 플러스친구팀입니다. 
버튼이 대량으로 설정된 채팅방 진입시 오류 발생 가능성이 발견되어 
시스템 안정화의 일환으로 API 스펙을 변경할 예정입니다. 
관련 내용에 대해 사전 안내 드리오니 잘 참고하시어 이용에 불편이 없으시길 바랍니다. 

▶︎ 변경 적용일 
2018년 1월 17일(수) 

▶︎ 변경 내용 
한 단계에서 표시될 수 있는 Keyboard Object 버튼 개수를 최대 100개로 제한 (현재는 제한 없음) 

▶︎ 참고 사항 
- 동작에 문제가 발생하지 않도록 변경 적용일 전까지 설계된 질문 및 답변을 미리 점검해주시기 바랍니다. 
- 변경 적용일 이후로는 100개를 초과하여 버튼을 설정했다 하더라도 채팅방에서는 100개까지만 표시됩니다. 
  (버튼은 별도의 정렬 없이 전달되는 JSON Array의 아이템 순서대로 100개까지만 표시) 
- 사용자가 버튼을 등록할 수 있도록 설계하신 경우, 위 내용을 메시지 등으로 미리 안내해주시는 것을 권장해드립니다. 

원활한 서비스 제공을 위해 더욱 노력하겠습니다. 
감사합니다.
```
