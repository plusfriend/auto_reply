```
[안내] 서버 플러그인 업데이트 영향에 따른 오류 해결 방법 안내

안녕하세요.  

지난 1월 15일(월) 기존 서버의 치명적인 오류 해결을 위해 서버 업데이트를 진행하였습니다.

이 영향으로 POST요청("API_URL/message")에서 "301 Moved Permanently"가 
발생한 경우 redirect를 못하고 있는 상황입니다. (GET요청 제외)

이 경우 이전된 URL로 API형 앱 URL을 수정하시면 정상 작동하게 되니 참고해주시기 바랍니다.
